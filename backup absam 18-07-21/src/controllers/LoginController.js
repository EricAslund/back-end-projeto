...
export default class LoginController {
    ...
    async login(req, res, next) {
        ...
        try {
            const [usuario] = await db('usuario').where('email', '=', email)

            if (usuario) {

                if (await bcrypt.compare(senha, usuario.senha)) {

                    const [estabelecimento] = await db('estabelecimento').where('estabelecimento.usuario_id', usuario.usuario_id)
                    ...
                    if (usuario.email_validado == true) {
                        ...
                        const token = await jwt.sign({ id: usuario.usuario_id, esta_id: estabelecimento.estabelecimento_id }, authConfig.ACCESS_TOKEN_SECRET, {
                            expiresIn: '8h',
                        });

                        return res.status(200).json({ usuario, token })

                    } else if (usuario.token_expiracao > now) {
                        const { token, now } = tokenEmail();
                        ...
                        return res.status(400).json({ mensagem: 'Verifique seu email para valida-lo.', status: 'error', title: '' })
                    } else {
                        return res.status(400).json({ mensagem: 'Verifique seu email para valida-lo.', status: 'error', title: '' })
                    }
                }
                return res.status(400).json({ mensagem: 'Senha invalida', status: 'error', title: '' });
            } else {
                return res.status(400).json({ mensagem: 'Email invalido', status: 'error', title: '' })
            }
